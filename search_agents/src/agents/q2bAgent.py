...
def heuristics(gameState: GameState):
    ...
    nearestFood = inf
    if len(foodList) > 20:
        reducedFoodList = random.sample(foodList, 20)
    else:
    ...
    heuristic = 9/nearestFood - 30/nearestGhost + 190/nearestScaredGhost + 30/nearestCapsule - len(foodList)*10 - len(capsuleList)*50
